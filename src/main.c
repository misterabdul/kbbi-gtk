...
populate_tree_model(GtkTreeStore* store)
{
  ...
  for (int i = 0; i < 100; i++) {
    gtk_tree_store_append(
      store, &iter1, NULL); /* Acquire a top-level iterator */
...
on_activate(GApplication* app, gpointer user_data)
{
  ...
  gtk_window_set_title(GTK_WINDOW(win), "KBBI Offline");
  ...
  gtk_scrolled_window_set_min_content_width(GTK_SCROLLED_WINDOW(scr), 300);
  gtk_container_add(GTK_CONTAINER(scr), lv);

  wv = webkit_web_view_new();
  gtk_widget_set_size_request(wv, 300, 300);
  gtk_widget_set_valign(wv, GTK_ALIGN_FILL);
  ...
  gtk_grid_attach(GTK_GRID(grid), search, 0, 0, 2, 1);
  gtk_grid_attach(GTK_GRID(grid), scr, 0, 1, 1, 1);
  gtk_grid_attach(GTK_GRID(grid), wv, 1, 1, 1, 1);
...
main(int argc, char** argv)
{
  ...
  app = gtk_application_new("io.github.misterabdul.kbbi-gtk",
